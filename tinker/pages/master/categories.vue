<script setup>
  ...
  onMounted(()=>{ navTitle().name = 'Master Categories'; });
</script>

<template>
  <div id="mytable"></div>

  <Modal id="myModal" title="Categories data">
    <form id="myForm">
      <Input type="text" name="_id" label="Id" style="display:none;"/>
      <Input type="text" name="category" label="Kategori" />
      <Input type="text" name="subcategory" label="Sub Kategori" />
        ...
        <span v-if="upl_img" :id="`showfile-upl`">
          <div class="grid grid-cols-12 gap-4">
            <div v-for="(item, index) in upl_img" class="col-span-12 mb-5">
              ...
              <img :src="`${appcfg.awsupl.bucketuri}img/categories/${upl_id}/${item.filename}`" alt="upload-image" class="rounded-lg m-3 h-40 w-40 object-cover">
            </div>
          </div>
      ...
      try {
        ...
        this.xdt.init({
          ...
          columns: [
            ...
            { label: 'Sub Kategori', field: 'subcategory' },
    ...
    methods:{
      async save_form(){
        ...
        try {
          ...
          let res = '';
          if(obj._id == ""){
            res = await call.post(`/api/master/categories`, obj);
          }else{
            res = await call.patch(`/api/master/categories/${obj._id}`, obj);
          }
      ...
      async del_record(){
        if (confirm('Hapus data ?')) {
          ...
          try {
            let obj = dataForm('myForm');
            let res = '';
            res = await call.delete(`/api/master/categories/${obj._id}`);
            if(res.status.code == 200){
      ...
      async savefile(record){
        try{
          let sav = await call.put(`/api/master/categories/${record._id}`, JSON.stringify(record));
      ...
      async upload(event, id){
        if (id==''){
          ...
          formData.append("key", "img/categories/"+id+"/"+fileName);
      ...
      async delFile(event, index){
        try {
          ...
          if(confirm('Delete file?')){
            let res = await call.patch(`/api/master/categories/${this.upl_id}`, data);
            if(res.status.code == 200){
