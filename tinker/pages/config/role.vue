  ...
  >
    <template #form>
      <div class="brf-section">
        <div class="brf-section-label">Role Details</div>
        <div class="brf-grid-2">
          <div class="brf-field">
            <label class="brf-label" for="code">Code</label>
            <input type="text" id="code" name="code" class="brf-input" />
          </div>
          <div class="brf-field">
            <label class="brf-label" for="name">Role Name</label>
            <input type="text" id="name" name="name" class="brf-input" />
          </div>
          <div class="brf-field">
            <label class="brf-label" for="option">option</label>
            <input type="text" id="option" name="option" class="brf-input" />
          </div>
        </div>
      </div>
    </template>
  ...
  export default {
    ...
    methods: {
      async onSave({ action, data }) {
        ...
        try {
          const res = action === 'add'
            ? await call.post('/api/config/role', data)
            : await call.patch('/api/config/role/' + data._id, data);
          res.status.code == 200
      ...
      async onDelete({ data }) {
        ...
        loading(true);
        try {
          const res = await call.delete('/api/config/role/' + data._id);
          res.status.code == 200
